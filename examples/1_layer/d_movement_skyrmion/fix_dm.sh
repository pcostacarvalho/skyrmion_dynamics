rm -f dij.#NAME.correct
 
#gsed  's/\./,/g' dij.out| awk '{printf("%8s%8s  %12.6f%12.6f%12.6f  %12.6f%12.6f%12.6f %12.6f\n", $1, $2, $3/3.89, $4/3.89, $5/3.89, $6, $7, $8, $9/3.89)}' > dij.aux

awk '{printf ("%1s %1s%12s%12s%12s\n", $1, $2, $3, $4, $5)}' dij.#NAME > aux
...
gsed 's/\./,/g' dij.#NAME | awk '{printf("%12s%12s%12s%12s\n", $6, $7, $8, $9)}' > aux.2
gsed -i 's/\,/./g' aux.2

paste aux aux.2 > dij.#NAME.correct
rm aux aux.2
